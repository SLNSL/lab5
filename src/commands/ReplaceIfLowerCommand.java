...
/**
 * Команда которая заменяет значение по ключу, если новое значение меньше старого
 */
public class ReplaceIfLowerCommand implements Command {
    ...
    public ReplaceIfLowerCommand(Collection collectionManager, Checker fieldsChecker, Asker asker, Messenger messenger, Printable printer) {
        this.collectionManager = collectionManager;
        this.fieldsAsker = asker;
    ...
    public boolean execute(String argument) throws IncorrectNumberOfArgumentsException{
        if(argument.isEmpty()) throw new IncorrectNumberOfArgumentsException(
                messenger.generateIncorrectNumberOfArgumentsMessage("replace_if_lower", 1));
        ...
        if (keyResult.hasError()) {
            ...
            if (productResult.hasError()) {
                ...
                Result<Boolean> replaceOrNot = collectionManager.replaceIfLower(keyResult.getResult(), (Product) productResult.getResult());
                ...
                printer.println(messenger.commandIsFinished("replace_if_lower"));
                return true;
            }
